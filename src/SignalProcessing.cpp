...
vector<double> SignalProcessing::hannWindow(const int N) {
    /*
    N: window length
...
vector<double> SignalProcessing::tukeyWindow(const int N, const double alpha) {
    /*
...
complex<float>* SignalProcessing::hilbert(const float* signal, const int N ) {
    /*
    ...
    */
    ...
    for(int i=0; i<N; i++) {
        ...
        temp[i] = complex<float>(signal[i],0.0);
    }
...
complex<float>* SignalProcessing::hilbert(const complex<float>* signal, const int N) {
    /*
    ...
    */
    ...
    kiss_fft_cpx ft1[N];
    kiss_fft_cpx ft2[N];
    complex<float>* output = new complex<float>[N];
    ...
    for(int i = 0; i < N; i++) {
        output[i] = complex<float>(signal[i].real() - ft1[i].i/N,signal[i].imag() + ft1[i].r /N) ;
...
float* SignalProcessing::calculateEnvelope(float *spectrum,size_t length) {
        /*
    ...
    void SignalProcessing::equalizeEnvelopeInPlace(float** spectra,size_t N,size_t M) {
        /*
    ...
    void SignalProcessing::stretchSpectraInPlace(float** spectra, float* referenceSpectrum, float minimumReferencePower) {
        /*
    ...
    void SignalProcessing::preprocessSpectrumInPlace(float** spectra, float* offsetSpectrum, float* chirp, float* referenceSpectrum,float* window   ) {
        /*
